...
sendAgain:
	...
	switch(method){
		...
		case DAV_REQ_NEW_COLLECTION:
			...
			newPath = g_strconcat(davPath, g_uri_escape_string(displayname, NULL, FALSE), NULL);
			if(newPath== NULL) goto failedRequest;
			req = ne_request_create(sess, "MKCOL", newPath);
...
int pushCard(sqlite3 *ptr, char *card, int addrBookID, int existing, int oldID){
	...
	switch(ret){
		...
		case 400:
			...
			if(postPushCard(ptr, sess, srvID, addrBookID, newID, oldID, existing) != 1){
				ret = -1;
			} else {
				ret = 1;
				setSingleInt(ptr, "contacts", "flags", 0, "contactID", newID);
